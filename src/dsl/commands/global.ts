...
export async function handleGlobal(node: AstNode, ctx: CommandContext): Promise<void> {
    ...
    if (valueExpression.startsWith('input:')) {
        ...
        try {
            const filePath = ctx.resolvePath(ctx.context.interpolate(valueExpression.substring(6).trim()));
        ...
        const spinner = new SimpleSpinner(`Executing ${node.type} at line ${node.line}`).start();
        try {
            const url = valueExpression.substring(6).trim();
    ...
    await ctx.globalContext.set(varName, finalValue);
    ...
    console.log(chalk.magenta(`[GLOBAL] ${varName} = ${valueForLog} saved!`));
}